...
class NeuconWRenderer:
    ...
    def uniform_sample(self, rays_o, rays_d, near, far, n_samples):
        device = rays_o.device
        ...
        if self.perturb > 0:
            ...
            z_vals = z_vals + (far - near) * t_rand * 2.0 / n_samples  # todo fix behind camera
    ...
    def volsdf_sample(self, rays_o, rays_d, near, far):
        ...
        beta0 = self.distribution.get_beta(rays_o).detach()

        # Start with uniform sampling
        z_vals = self.uniform_sample(rays_o, rays_d, near, far, n_samples_eval)
        ...
        while not_converge and total_iters < max_total_iters:
            ...
            density = self.distribution.density_func(sdf.reshape(z_vals.shape), beta=beta.unsqueeze(-1))
            ...
            if (not_converge and total_iters < max_total_iters) or (self.perturb <= 0):
    ...
    def get_error_bound(self, beta, sdf, z_vals, dists, d_star):
        ...
        density = self.distribution.density_func(sdf.reshape(z_vals.shape), beta=beta)
